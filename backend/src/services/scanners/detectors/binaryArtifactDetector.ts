...
const BUILD_DIR_PATTERN = /(^|\/)(dist|build|target|bin|out|release|debug|vendor|third_party|third-party|\.gradle|\.m2|obj|lib)(\/|$)/i;
...
export const binaryArtifactDetector: Detector = {
  ...
  detect({ files }) {
    ...
    for (const file of files) {
      ...
      if (!BINARY_EXTENSIONS.has(ext) && !isProbablyBinary(file)) {
        continue;
      }
      ...
      const ruleId = executable
        ? inUnusualDir || hidden
          ? "binary.executable.unusual-location"
          : "binary.executable.package"
        : packaged
          ? inUnusualDir || hidden
            ? "binary.packaged.embedded-artifact"
            : "binary.packaged.source-artifact"
          : "binary.payload.candidate";
      ...
      const rationale = inBuildDir
        ? "File nhị phân nằm trong thư mục build hoặc output, nên có thể là artifact hợp lệ được sinh ra trong quá trình build."
        : inUnusualDir || hidden
          ? "File nhị phân hoặc packaged artifact xuất hiện lẫn trong source tree/hidden path. Vị trí này không phải nơi thường dùng để lưu executable hoặc dependency đóng gói sẵn."
          : executable
      ...
      findings.push(
        createFinding({
          ...
          recommendation: inBuildDir
            ? "Xác minh artifact này thực sự là output build hợp lệ. Nếu không cần commit, cân nhắc loại khỏi repository và tạo lại từ pipeline build."
            : "Xác minh nguồn gốc file, kiểm tra checksum hoặc provenance, và cân nhắc thay bằng dependency/build step tái tạo được thay vì commit file nhị phân sẵn.",
          falsePositiveNote: inBuildDir
            ? "Artifact nằm trong build/output dir nên có thể hợp lệ, đặc biệt với repo phân phối binary mẫu hoặc fixture build."
          ...
          severity,
          confidence: inUnusualDir || hidden ? 0.88 : inBuildDir ? 0.42 : executable ? 0.82 : 0.7,
