...
export const workflowRiskDetector: Detector = {
  ...
  detect({ files }) {
    ...
    for (const file of files) {
      if (!file.content || !/\.github\/workflows\/.*\.ya?ml$/i.test(file.relativePath)) {
        continue;
      }

      if (!WORKFLOW_PATTERN.test(file.content)) {
        continue;
      }
      ...
      findings.push(
        createFinding({
          ...
          recommendation: "Kiểm tra trigger, quyền token, và command chạy trong workflow; hạn chế remote execution và tắt persist-credentials nếu không cần thiết.",
