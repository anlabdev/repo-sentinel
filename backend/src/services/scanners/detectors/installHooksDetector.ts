...
export const installHooksDetector: Detector = {
  ...
  detect({ files }) {
    ...
    for (const file of files) {
      if (file.relativePath !== "package.json" || !file.content) continue;
      ...
      try {
        ...
        for (const [scriptName, scriptValue] of Object.entries(parsed.scripts ?? {})) {
          if (!INSTALL_SCRIPTS.includes(scriptName)) continue;

          const suspicious = SUSPICIOUS_TERMS.test(scriptValue);
          findings.push(createFinding({
            ruleId: suspicious ? "installHooks.suspicious-command" : "installHooks.present",
            ...
            recommendation: suspicious
              ...
              : "Review mục đích của hook, tài liệu hóa rõ lý do tồn tại, và tránh để hook tự chạy nếu không thật sự cần thiết.",
